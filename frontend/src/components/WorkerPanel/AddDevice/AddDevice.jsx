...
export default function AddDevice() {
    ...
    return (
        <div className={styles.add_device__wrapper}>
            ...
            <Button className={styles.add_device__button} onClick={() => { setAddEspDialog(true) }}>Добавить устройство</Button>
            <AddEspDialog open={addEspDialog} setDialog={(event) => { setAddEspDialog(event) }} findUser={findUser} ></AddEspDialog>
            {devicesArray[0] !== null && devicesArray[0] !== undefined && devicesArray.length > 0 && <div className={styles.add_device__main__container}>
                {devicesArray.map(item =>
                    <div className={styles.item_container} key={item.device_uid} onClick={() => editDevice(item)}>
                        <h4 className={styles.item_container__header}>Устройство: {item.device_uid}</h4>
            ...
            {devicesArray[0] === null && <div className={styles.user_not_found}><h4>Пользователь не найден</h4></div>}
            {devicesArray.length === 0 && <div className={styles.no_devices}><h4>Устройств нет</h4></div>}
            {open && <DeviceDialog
